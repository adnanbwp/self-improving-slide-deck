...
- **2026-07-08**: `find_thumbnail()` searched the *shared* `screenshots/` before the deck's own folder and matched on **version only** — so agentic-engineering v1.1.0 got served hybrid-delivery's orphaned `v1.1.0-slide1.png` (hybrid was once v1.1.0). Version numbers collide across decks. **Fixed** by searching `deck_dir/screenshots` first. Also: `push.py` cache-busts the thumbnail with `?v={version}` — if you fix a thumbnail *without* bumping the version, browsers keep the stale image (same URL). Bump the version or the fix won't be visible. Keep the shared `screenshots/` dir clean of version-named orphans.
