...
4. **Personalize the scaffold (one-time, on first activation only).** The scaffold ships with `Adnan` placeholders in a handful of files where the prose names the user as the actor. Detect this:
   - Run `grep -rl "Adnan" .` (or your tool's equivalent). If zero hits, the scaffold is already personalized — skip to step 5.
   ...
   - Replace every `Adnan` token across all `.md`, `.yaml`, `.yml`, `.txt` files in the scaffold with the captured value. In-place edits, no backups needed (git tracks history).
...
- **EXISTING FILES TOUCHED:** list any existing files you modified (should be empty unless the user asked for something specific, OR a CLAUDE.md/GEMINI.md/etc. that pre-existed and needed the identity overlay added, OR personalization-substitution edits across files where `Adnan` lived)
- **PERSONALIZATION:** confirm whether you ran the one-time `Adnan` substitution (yes / skipped — already personalized), the user's first name captured (or "n/a"), and the count of tokens replaced
