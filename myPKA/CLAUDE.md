...
Specialists are bound as host subagents in `.claude/agents/<slug>.md` (one level up, at the workspace root). Larry dispatches them via Claude Code's `Agent` tool with `subagent_type: <slug>`. Multiple specialists run in parallel when called from a single message.
